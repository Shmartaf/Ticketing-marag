...
import { TextField, FormControl, InputLabel, Select, MenuItem, Button, FormControlLabel, RadioGroup, Radio, Checkbox } from '@mui/material';
...
const DynamicBoardForm = ({ onCreateBoard }) => {
    ...
    const handleAddField = () => {
        if (columnName.trim() !== '') {
            setBoardFields([...boardFields, { name: columnName, type: columnType, options: columnOptions }]);
            setColumnName('');
            setColumnOptions([]);
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div>
                ...
                <Button variant="contained" onClick={handleAddField} style={{ marginLeft: '10px' }}>
                    Add Field
                </Button>
            </div>
            <div style={{ marginTop: '20px' }}>
                {boardFields.map((field, index) => (
                    <div key={index} style={{ marginBottom: '10px' }}>
                        ...
                        <Button variant="outlined" onClick={() => handleRemoveField(index)} style={{ marginLeft: '10px' }}>
                            Remove
                        </Button>
                    </div>
                ))}
            ...
            <div style={{ marginTop: '20px' }}>
                <Button variant="contained" type="submit">Create Board</Button>
            </div>
        </form>
    );
};
